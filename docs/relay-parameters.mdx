...
Use one of these two layouts and keep the start command aligned with the chosen root:

| Railway Root Directory | Build Config Source | Start Command |
| --- | --- | --- |
| repo root (`.`) | `nixpacks.toml` | `node scripts/ais-relay.cjs` |
| `scripts` | `scripts/nixpacks.toml` | `node ais-relay.cjs` |

Do not combine `Root Directory = scripts` with `Start Command = node scripts/ais-relay.cjs`.
That resolves to `/app/scripts/ais-relay.cjs` inside the container, but the real file is `/app/ais-relay.cjs` when `scripts/` is the build root.
