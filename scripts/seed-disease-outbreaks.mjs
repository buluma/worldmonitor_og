...
import { loadEnvFile, CHROME_UA, runSeed, resolveProxy, fetchTextWithProxyFallback } from './_seed-utils.mjs';
...
async function fetchRssItems(url, sourceName, proxyAuth = '') {
  try {
    const xml = await fetchTextWithProxyFallback(url, proxyAuth, {
...
async function fetchDiseaseOutbreaks() {
  const [whoItems, cdcItems, outbreakNewsItems, tghItems] = await Promise.all([
    ...
    fetchRssItems(OUTBREAK_NEWS_FEED, 'Outbreak News Today', _proxyAuth),
