...
function htmlVariantPlugin(meta: VariantMeta, variant: string, isDesktopBuild: boolean): Plugin {
  return {
    name: 'html-variant',
    transformIndexHtml(html) {
      let result = html
        ...
        .replace(/<meta name="subject" content=".*?" \/>/, `<meta name="subject" content="${meta.subject}" />`)
        .replace(/<meta name="classification" content=".*?" \/>/, `<meta name="classification" content="${meta.classification}" />`)
      ...
      if (variant !== 'full') {
        result = result.replace(
          ...
          `v='${variant}';document.documentElement.dataset.variant=v;`
        );
      }
      ...
      if (variant !== 'full') {
        result = result
          ...
          .replace(/\/favico\/android-chrome/g, `/favico/${variant}/android-chrome`)
          .replace(/\/favico\/og-image/g, `/favico/${variant}/og-image`);
      }
